...
If it's possible for an individual message to be sent by the producer instances (possibly as the result of some higher level retry strategy) then duplicates are possible and the transactional semantics are lost.
